...
typedef enum {
    VN_NONE,
    VN_STR_LEN, VN_STR_EQ, VN_STR_NE, VN_STR_CONCAT,
} ValueIntr;

static ValueIntr cg_value_intrinsic(const SV callee) {
    if (sv_eq(callee, (SV){ "str_len", 7 })) return VN_STR_LEN;
    if (sv_eq(callee, (SV){ "str_eq", 6 })) return VN_STR_EQ;
    if (sv_eq(callee, (SV){ "str_ne", 6 })) return VN_STR_NE;
    if (sv_eq(callee, (SV){ "str_concat", 10 })) return VN_STR_CONCAT;
    return VN_NONE;
}

/* Emit a C expression for a value-returning string intrinsic. */
static void cg_value_intrinsic_call(CG *cg, Expr *e, ValueIntr kind, SB *out) {
    Expr *a0 = vec_get(&e->as.call.args, 0);
    switch (kind) {
    case VN_STR_LEN:
        sb_appendcstr(out, "(int)strlen(");
        cg_expr(cg, a0, out);
        sb_append_char(out, ')');
        break;
    case VN_STR_EQ:
        sb_appendcstr(out, "(strcmp(");
        cg_expr(cg, a0, out);
        sb_appendcstr(out, ", ");
        cg_expr(cg, vec_get(&e->as.call.args, 1), out);
        sb_appendcstr(out, ") == 0)");
        break;
    case VN_STR_NE:
        sb_appendcstr(out, "(strcmp(");
        ...
        break;
    case VN_STR_CONCAT:
        sb_appendcstr(out, "cpm_str_concat(");
        cg_expr(cg, a0, out);
        sb_appendcstr(out, ", ");
        cg_expr(cg, vec_get(&e->as.call.args, 1), out);
        sb_append_char(out, ')');
...
static void cg_expr(CG *cg, Expr *e, SB *out) {
    ...
    case EX_CALL: {
        ...
        if (!cg_is_user_fn(cg, callee)) {
            const char *fmt = cg_intrinsic_fmt(callee);
            if (fmt) {
                cg_intrinsic_call(cg, e, fmt, out);
                break;
            }
            ValueIntr vi = cg_value_intrinsic(callee);
            if (vi != VN_NONE) {
                cg_value_intrinsic_call(cg, e, vi, out);
                break;
            }
        }
...
int codegen_program(Program *prog, Diag *d, Arena *a, SB *out) {
    ...
    sb_appendcstr(out, "#include <string.h>\n");
    sb_appendcstr(out, "#include <stdlib.h>\n\n");
    sb_appendcstr(out, "static char *cpm_str_concat(const char *a, const char *b) {\n");
    sb_appendcstr(out, "    size_t la = strlen(a), lb = strlen(b);\n");
    sb_appendcstr(out, "    char *r = (char *)malloc(la + lb + 1);\n");
    sb_appendcstr(out, "    if (!r) return NULL;\n");
    sb_appendcstr(out, "    memcpy(r, a, la); memcpy(r + la, b, lb);\n");
    sb_appendcstr(out, "    r[la + lb] = '\\0';\n");
    sb_appendcstr(out, "    return r;\n");
    sb_appendcstr(out, "}\n\n");
